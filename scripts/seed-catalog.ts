...
function createProducts(catMap: Record<string, string>) {
  ...
  ]; return pd.map(p => ({...p, variants: p.variants.map(v => ({...v, images: v.images.length ? v.images : p.images.map(u => ({url:u, publicId:u.replace(/[^a-zA-Z0-9]/g,"_"), format:"jpg", width:800, height:1000})) })) })); }
